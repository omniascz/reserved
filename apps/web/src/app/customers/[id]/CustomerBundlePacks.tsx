...
export function CustomerBundlePacks({ customerId }: { customerId: string }) {
  const [allocations, setAllocations] = useState<AdminCustomerBundlePack[]>([]);
  const [templates, setTemplates] = useState<AdminBundlePack[]>([]);
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const [allocs, tpls] = await Promise.all([
        listCustomerBundlePacks(customerId),
        listBundlePacks(),
      ]);
  ...
  async function handleAllocate(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const payload: Parameters<typeof allocateBundlePack>[1] = {
        bundlePackId: selectedTemplate,
      };
      if (priceOverride) {
        payload.pricePaidHellers = Math.round(Number(priceOverride) * 100);
      }
      if (note) payload.note = note;
      await allocateBundlePack(customerId, payload);
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
      <div className="flex justify-between items-center mb-3">
        <h3 className="font-semibold">Bundle balíčky ({allocations.length})</h3>
        <button
              ...
              return (
                <tr key={a.id} className="border-b border-slate-50">
                  ...
                  <td className="py-2 text-slate-600 text-xs">
                    {a.itemsRemaining.map((it, i) => (
                      <div key={i}>{it.quantity}× pol.</div>
                    ))}
                    <div className="font-medium">Celkem: {totalRemaining(a.itemsRemaining)}</div>
                  </td>
                  <td className="py-2 text-slate-600 text-xs">
                    {a.validUntil
