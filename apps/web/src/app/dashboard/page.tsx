...
export default function DashboardPage() {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const [o, pd, s, em, cu, cp] = await Promise.all([
  ...
  const completionRate =
    overview && overview.totalBookings > 0
      ? Math.round((overview.completedCount / overview.totalBookings) * 100)
      : 0;
  const noShowRate =
    overview && overview.totalBookings > 0
      ? Math.round((overview.noShowCount / overview.totalBookings) * 100)
      : 0;
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
              <KpiCard
                ...
                hint={`${overview?.uniqueCustomers ?? 0} unikátních klientů`}
              />
              <KpiCard
                label="Tržby"
                value={`${(((overview?.revenueHellers ?? 0) / 100) | 0).toLocaleString('cs-CZ')} Kč`}
                hint="Z dokončených i očekávaných"
              />
              <KpiCard
                label="Dokončeno"
                value={`${completionRate}%`}
                hint={`${overview?.completedCount ?? 0} rezervací`}
                color="emerald"
              />
              <KpiCard
                label="No-show"
                value={`${noShowRate}%`}
                hint={`${overview?.noShowCount ?? 0} klientů nedorazilo`}
                  ...
                  <div style={{ width: '100%', height: 280 }}>
                    <ResponsiveContainer>
                      <PieChart>
                        <Pie
                          data={[
                            { name: 'Dokončeno', value: overview.completedCount },
                            { name: 'Potvrzeno', value: overview.confirmedCount },
                            { name: 'Zrušeno', value: overview.cancelledCount },
                            { name: 'No-show', value: overview.noShowCount },
                          ].filter((d) => d.value > 0)}
                          dataKey="value"
                          nameKey="name"
                          cx="50%"
